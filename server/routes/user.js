...
router.patch('/change', async (req, res) => {
    ...
    await User.changePassword(oldPassword, newPassword, function(err) {
...
router.delete('/delete', async (req, res) => {
    if(!req.isAuthenticated()) return res.status(403).json({message: "User not loginned"});
...
router.post('/picture', upload.single('file') , async (req, res) => {
    try {
        ...
        await User.updateOne({ _id: req.user._id, }, { $set: { logo: file } }, (err)=>{
            if(err) return res.status(404).json(err);
            else return res.status(200).json();
        })
    } catch (error) {
